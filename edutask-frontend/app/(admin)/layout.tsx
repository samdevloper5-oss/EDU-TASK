...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-background flex">
      <aside className="hidden md:flex flex-col w-56 h-screen fixed left-0 top-0 bg-card border-r border-border z-40">
        <div className="px-5 py-5 border-b border-border">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-muted-foreground text-[10px]">EduTask</p>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5">
          {adminNav.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-muted-foreground hover:text-foreground hover:bg-muted transition-colors font-medium"
            >
        ...
        <div className="px-3 py-4 border-t border-border">
          <div className="px-3 py-2 mb-1">
            ...
            <p className="text-[10px] text-subtle">Administrator</p>
          </div>
