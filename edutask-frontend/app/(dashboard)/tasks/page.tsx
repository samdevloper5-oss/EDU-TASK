...
export default function TasksPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <p className="text-label mb-2">Marketplace</p>
        <h1 className="text-2xl font-bold text-foreground tracking-tight" style={{ fontFamily: 'var(--font-heading)' }}>Tasks</h1>
      </div>
          ...
          {taskWithDeadline.map((task: any) => (
            <LazyTaskCard key={task.id}>
              <Link href={`/tasks/${task.id}`} className="block">
                <Card className="h-full p-5 transition-colors hover:border-primary/20">
                  <div className="mb-3 flex items-start justify-between gap-3">
                    ...
                    <span className={`flex items-center gap-1 text-xs ${task.daysLeft < 1 ? 'text-destructive' : task.daysLeft <= 3 ? 'text-warning' : 'text-muted-foreground'}`}>
                  ...
                  <p className="mt-2 line-clamp-2 text-xs text-muted-foreground leading-relaxed">{task.description}</p>

                  <div className="mt-4 flex flex-wrap gap-1">
                    {(task.required_skills ?? []).slice(0, 3).map((skill: string) => (
                      <span key={skill} className="rounded bg-muted px-2 py-0.5 text-[10px] font-medium text-muted-foreground">
                        {skill}
                      </span>
                    ))}
                  </div>
