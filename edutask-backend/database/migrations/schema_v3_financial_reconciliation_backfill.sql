-- One-time / idempotent reconciliation backfill.
-- Purpose:
--   Align wallets snapshot balances with authoritative ledger_entries.
-- Safety:
--   - Inserts only when a wallet has non-zero delta.
--   - Produces balanced journals per wallet.
--   - Re-running is safe (no-op when deltas are zero).
...
SELECT
  ...
  jsonb_build_object('source', 'schema_v3_financial_reconciliation_backfill', 'at', NOW()),
  NULL
FROM entries
